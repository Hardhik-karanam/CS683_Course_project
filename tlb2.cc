...
namespace ParametricDramDirectoryMSI
{
	...
	TLB2::TLB2(String name, String cfgname, core_id_t core_id, ComponentLatency access_latency, UInt32 num_entries, UInt32 associativity, int *page_size_list, int page_sizes, String tlb_type, bool allocate_on_miss, bool prefetch, TLBPrefetcherBase **tpb, int _number_of_prefetchers, int _max_prefetch_count)
	...
	CacheBlockInfo *TLB2::lookup(IntPtr address, SubsecondTime now, bool model_count, Core::lock_signal_t lock_signal, IntPtr eip, bool modeled, bool count, PageTable *pt)
	{


        std::cout << "[DEBUG] Entering TLB2::lookup, Address: " << std::hex << address << std::endl;//added by me
		...
		if (hit)
		{
			std::cout << "[DEBUG] Cache hit for address: " << std::hex << address << std::endl;//added by me
			tlb_stats.m_hit++;
			return hit;
		}
		else std::cout << "[DEBUG] Cache miss for address: " << std::hex << address << std::endl; //added by me
	...
	std::tuple<bool, IntPtr, int> TLB2::allocate(IntPtr address, SubsecondTime now, bool count, Core::lock_signal_t lock_signal, int page_size, IntPtr ppn, bool self_alloc)
	{
		std::cout << "[DEBUG] TLB2::allocate invoked for address: " << address
          << " with page size: " << page_size << std::endl; // added by me
