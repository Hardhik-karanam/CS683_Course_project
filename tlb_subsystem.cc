...
namespace ParametricDramDirectoryMSI
{
    ...
    TLBHierarchy::TLBHierarchy(String name, Core *core, MemoryManager *memory_manager, ShmemPerfModel *shmem_perf_model)
    {
        ...
        for (int level = 1; level <= numLevels; ++level)
        {
            ...
            for (int tlbIndex = 1; tlbIndex <= numTLBs; ++tlbIndex)
            {
                ...
                String type = Sim()->getCfg()->getString(tlbconfigstring + "/type");
                int size = Sim()->getCfg()->getInt(tlbconfigstring + "/size");
                int assoc = Sim()->getCfg()->getInt(tlbconfigstring + "/assoc");
                page_sizes = Sim()->getCfg()->getInt(tlbconfigstring + "/page_size");
                page_size_list = (int *)malloc(sizeof(int) * page_sizes);
                bool allocate_on_miss = Sim()->getCfg()->getBool(tlbconfigstring + "/allocate_on_miss");
                ComponentLatency latency = ComponentLatency(core->getDvfsDomain(), Sim()->getCfg()->getInt(tlbconfigstring + "/access_latency"));

                for (int i = 0; i < page_sizes; i++)
                    page_size_list[i] = Sim()->getCfg()->getIntArray(tlbconfigstring + "/page_size_list", i);
                ...
                if (level == 2)
				{
                    std::cout << "[DEBUG] Instantiating TLB2 at level: " << level << std::endl;
					TLB2 *tlb = new TLB2(tlbname, tlbconfigstring, core->getId(), latency, size, assoc, page_size_list, page_sizes, type, allocate_on_miss);
					tlbLevels[level - 1].push_back(reinterpret_cast<TLB*>(tlb));
					if (type == "Data")
						data_path[level - 1].push_back(reinterpret_cast<TLB*>(tlb));
					else if (type == "Instruction")
						instruction_path[level - 1].push_back(reinterpret_cast<TLB*>(tlb));
					else
					{
						data_path[level - 1].push_back(reinterpret_cast<TLB*>(tlb));
						instruction_path[level - 1].push_back(reinterpret_cast<TLB*>(tlb));
        ...
        if (prefetch_enabled)
        {
            ...
            for (int pqIndex = 1; pqIndex <= numpqs; pqIndex++)
            {
                ...
                String type = Sim()->getCfg()->getString(pqconfigstring + "/type");

                page_size_list = (int *)malloc(sizeof(int) * 2);
                page_size_list[0] = 12;
                page_size_list[1] = 24;
                int size = Sim()->getCfg()->getInt(pqconfigstring + "/size");
                std::string pqName = "PQ_" + std::to_string(pqIndex);
                String pqname = String(pqName.begin(), pqName.end());
                int number_of_prefetchers = Sim()->getCfg()->getInt(pqconfigstring + "/number_of_prefetchers");
                ...
                {
                    String name = Sim()->getCfg()->getStringArray(pqconfigstring + "/prefetcher_list", i);
                    prefetchers[i] = TLBprefetcherFactory::createTLBPrefetcher(name, pqconfigstring, core, memory_manager, shmem_perf_model);
                }

                ComponentLatency latency = ComponentLatency(core->getDvfsDomain(), Sim()->getCfg()->getInt(pqconfigstring + "/access_latency"));
    ...
    {
        if (page_size_predictor == nullptr)
        {
            if (page_sizes == 0)
                return 0;
    ...
    {
        for (auto &tlb_list : tlbLevels)
            for (auto *tlb : tlb_list)
                delete tlb;
